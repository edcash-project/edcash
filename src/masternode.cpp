...
//MASTER
     if (Params().NetworkID() == CBaseChainParams::MAIN) {
         if (addr.GetPort() != 5003) return false;
     } else if (addr.GetPort() == 5003)
         return false;
//MASTER
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when EDC collateral tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
