...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a EDC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no edcash: URI
...
{
    // Convert edcash:// to edcash:
    //
    //    Cannot handle this later, because edcash:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open edcash.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "EdCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for EdCash.lnk
...
{
    return GetAutostartDir() / "edcash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a edcash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=EdCash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the edcash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add edcash app to startup item list
